  ...
  ws.on('message', (raw) => {
    try {
      ...
      if (type === 'join') {
        ...
        room.forEach((client, id) => { if (id !== socketId && client.ws.readyState === 1) client.ws.send(JSON.stringify({ type: 'canvas', payload })); });
      } else if (type === 'chat') {
        broadcastToRoom(roomId, { type: 'chat', payload });
      } else if (type === 'music') {
        room.forEach((client, id) => { if (id !== socketId && client.ws.readyState === 1) client.ws.send(JSON.stringify({ type: 'music', payload })); });
